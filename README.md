...
- **Formal verification is partial, and upstream says so.** The `quilt-verilog`
  README headlines "6/6 PASS" and then, in the *same sentence*, records that the
  run was at then-depths tick 80 / fair 80, that depths have since been raised to
  105/130, that no completed run at the new depth is on record, that the
  committed snapshot shows `fair INCOMPLETE@85`, and that the re-run exceeded
  18 minutes of solver time at step 87. The committed
  `formal/AUDIT-SNAPSHOT.json` (4 PASS / 4 INCOMPLETE / 2 UNKNOWN) is consistent
  with that caveat. Read the caveat, not just the headline.
